...
from data.chat import Base, User
...
class ChatAccessor:
    ...
    def clear_chat(self, self_id):
        with self.Session() as session:
            session.query(Chat).filter(or_(Chat.user1_id == self_id, Chat.user2_id == self_id)).delete()
            session.query(Message).filter(or_(Message.sent_by == self_id, Message.sent_to == self_id)).delete()
            session.commit()
    ...
    def get_all_message(self, id):
        messages = self.session.query(Message).filter(
            or_(Message.sent_by == id, Message.sent_to == id),
        ).all()
        return messages